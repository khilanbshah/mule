...
public class StereotypesDeclarationEnricher implements DeclarationEnricher {
  ...
  private static class EnricherDelegate {
      ...
      if (implementingType.isPresent()) {
        new IdempotentDeclarationWalker() {
          ...
          protected void onConstruct(WithConstructsDeclaration owner, ConstructDeclaration declaration) {
            // Only handling scope case for now
            declaration.getModelProperty(ImplementingMethodModelProperty.class)
                ...
                .ifPresent(methodElement -> {
                  addStereotypes(extensionDeclarer,
                                 methodElement.getDeclaringClass(),
                  ...
                  methodElement.getParameters().stream()
                      ...
                      .findFirst()
                      .ifPresent(callbackParameter -> declaration.getNestedComponents().stream()
                          .filter(component -> component instanceof NestedChainDeclaration)
                          .findFirst()
                          .ifPresent(chain -> addAllowedStereotypes(extensionDeclarer,
                                                                    callbackParameter,
                                                                    (NestedChainDeclaration) chain)));
                });
          }
    ...
    void addStereotypes(ExtensionDeclarer extensionDeclarer, Class<?> ownerClass, Method method,
                        ComponentDeclaration declaration, StereotypeModel defaultStereotype) {

      Stereotype stereotypes = method.getAnnotation(Stereotype.class);
      if (stereotypes == null) {
        addStereotypes(extensionDeclarer, ownerClass, declaration, defaultStereotype);
      } else {
    ...
    private void addStereotypes(ExtensionDeclarer extensionDeclarer, ComponentDeclaration declaration,
      ...
      if (customStereotype != null) {
        ...
        try {
          ...
          throw new IllegalModelDefinitionException("Invalid StereotypeProvider found with name "
